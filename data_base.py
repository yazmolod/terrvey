...
class DBController:
    ...
    def create_quest_tables(self):
        ...
        for i in common.JSON:
            ...
            if 'Quest' in name:
                ...
                elif i['type'] == 'opt+' or i['type'] == 'mopt+':
                    msg = """
                    CREATE TABLE IF NOT EXISTS '{}' (
                    'AnswerId' INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
                    'RespondentId' INTEGER,
            ...
            if table_name != 'Respondents':
                ...
                for i in range(len(table_values)):
	            	...
	            	# opt+ или mopt+
                        ...
                        value = value.replace('"', '""')
                        statements.append ('''INSERT INTO {} (RespondentId, AnswerText) VALUES ({}, "{}")'''.format (table_name, current_resp, value))
        for statement in statements:
            self.cursor.execute(statement)
        if commit:
            self.hard_commit()
    ...
    def delete_resp(self, resp_id):
        self.cursor.execute('DELETE FROM Respondents WHERE RespondentId = %d' % (resp_id))
        self.hard_commit()
